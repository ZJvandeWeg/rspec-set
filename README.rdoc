...
The following code will create one (and only one!) flight before running the examples and reload the flight from the DB before each example.
  ...
  describe Flight do
    ...
    it "should be on_time" do
      expect(flight).to be_on_time
    end

    it "should be cancellable" do
      flight.cancel

      expect(flight).to be_cancelled
    end

    it "should be delayable" do
      flight.delay
      expect(flight).to be_delayed
    end
  end
...
* make <tt>before(:all)</tt> running in a transaction - See: http://rhnh.net/2010/10/06/transactional-before-all-with-rspec-and-datamapper
